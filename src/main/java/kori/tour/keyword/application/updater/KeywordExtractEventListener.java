...
public class KeywordExtractEventListener {
	...
	/**
	 * 키워드는 많아봐야 네개라서, Batch query 보다, 이어서 바로 저장하는게 나을 것 같음. 어차피 AI 에게 요청을 Tour 단위로 하는
	 * 상황이니까 스레드를 빠르게 돌리는게 낫다고 판단 Todo: ExtractKeyword 에서 Retry를 했음에도 정상 응답을 못받았은 상황을 더
	 * 자세히 고려해야 한다.
	 */
	...
	public void listenKeywordExtractEvent(KeywordExtractingEvent event) {
		...
		log.info("KeywordExtractingEvent={} saved {} 개 Tour의 keyword 저장 Total time: {}ms", event.eventId(),
				event.newToursEntity().size(), (endTime - startTime));
	}
